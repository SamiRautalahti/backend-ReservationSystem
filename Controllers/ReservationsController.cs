...
namespace ReservationSystem2022.Controllers
{
    ...
    public class ReservationsController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<ReservationDTO>>> GetReservations() // haetaan kaikki varaukset
        {
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<Reservation>> GetReservation(long id) // haetaan varaus ID:n perusteella
        {
        ...
        [HttpPost]
        [Authorize]
        public async Task<ActionResult<ReservationDTO>> PostReservation(ReservationDTO reservation) // lisätään varaus
        {
            bool isAllowed = await _authenticationService.IsAllowed(this.User.FindFirst(ClaimTypes.Name).Value, reservation);
            if (!isAllowed)
            {
        ...
        [HttpDelete("{id}")]
        public async Task<IActionResult> DeleteReservation(long id) // poistetaan varaus ID:n perusteella
        {
            var reservation = await _context.Reservations.FindAsync(id);
            if (reservation == null)
            {
